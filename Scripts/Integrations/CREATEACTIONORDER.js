...
function doInputValidation() {
    var isPass = true;
    if (isBlank(requestType)) { addMessage("RequestType is required"); isPass = false; }
    if (isBlank(addressID) && isBlank(addressObj)) { addMessage("You have to input AddressID or AddressObj"); isPass = false; }
    if (isBlank(addressID) && !isBlank(addressObj)) {
        if (isBlank(addressObj.get("StreetNumber"))) { addMessage("AddressObj.StreetNumber is required"); isPass = false; }
        if (isBlank(addressObj.get("StreetName"))) { addMessage("AddressObj.StreetName is required"); isPass = false; }
        if (!Number(addressObj.get("StreetNumber"))) { addMessage("StreetNumber has to be numerical"); isPass = false; }
    }
    return isPass;
}
